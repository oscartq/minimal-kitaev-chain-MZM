...
def run_simulation(nqbit=4, nsteps=20, eps=[0,0], t=[1.0,1.0], Delta=1.0, tc=1.0):
    ...
    print(rf"Starting simulation with nqbit={nqbit}, theta range: -2$\pi$ to +$\pi$, nsteps={nsteps}")
    ...
    for idx, k in enumerate(range(0, nsteps + 1), start=1):
        ...
        Delta_aux = [Delta*np.exp(-1.0j*theta),Delta*np.exp(1.0j*theta)]
        ...
        dimers = [(0, 1), (2, 3)]
        for k, (i, j) in enumerate(dimers):
            # Hopping i<->j
            ham_aux += FermionicOp({f"+_{i} -_{j}": t[k]}, num_spin_orbitals=nqbit)
            circ_aux1 += f"{t[k]}[{i}^ {j}]+"
            ham_aux += FermionicOp({f"+_{j} -_{i}": np.conjugate(t[k])}, num_spin_orbitals=nqbit)
            circ_aux1 += f"{np.conjugate(t[k])}[{j}^ {i}]+"

            # Pairing +i +j and -j -i
            ham_aux += FermionicOp({f"+_{i} +_{j}": Delta_aux[k]}, num_spin_orbitals=nqbit)
            circ_aux1 += f"{Delta[k]}[{i}^ {j}^]+"
            ham_aux += FermionicOp({f"-_{j} -_{i}": np.conjugate(Delta_aux[k])}, num_spin_orbitals=nqbit)
            circ_aux1 += f"{np.conjugate(Delta[k])}[{j} {i}]+"
